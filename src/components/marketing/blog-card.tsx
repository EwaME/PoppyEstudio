    ...
    <Link href={`/blog/${post.slug}`} className="block">
      <Card className="gap-0 overflow-hidden pt-0 transition-shadow hover:shadow-lg">
        ...
        <CardContent className="space-y-2 pt-4">
          ...
          <p className="font-heading font-semibold hover:text-brand-primary-hover">{post.titulo}</p>
          {post.resumen && <p className="line-clamp-2 text-sm text-muted-foreground">{post.resumen}</p>}
          <span className="inline-block pt-1 text-sm font-medium text-brand-primary-hover hover:underline">
            Leer más →
          </span>
        </CardContent>
      </Card>
