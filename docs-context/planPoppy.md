...
El presente capítulo define el modelo de datos del sistema web de Poppy Crafty.

La base de datos será el núcleo del sistema y almacenará toda la información relacionada con productos, artículos del blog, categorías, imágenes, formularios de contacto y futuras funcionalidades como usuarios, pedidos y comercio electrónico.

Se utilizará **PostgreSQL** mediante **Supabase**, aprovechando sus capacidades de autenticación, almacenamiento, Row Level Security (RLS), funciones SQL y escalabilidad.
...
# 247. Arquitectura General

El modelo estará organizado en los siguientes dominios:
...
Todas las tablas seguirán las siguientes reglas.
...
# 253. Tabla Product Tag Relations
...
Permitirá generar recomendaciones manuales.

---

# 255. Tabla Posts

Representa los artículos del blog.
...
# 260. Tabla Media Files

Esta tabla administrará todos los archivos almacenados en Supabase Storage.
...
# 264. Tabla Quote Requests

Representa solicitudes de cotización.
...
# 266. Tabla SEO Redirects

Permitirá administrar redirecciones.
...
# 268. Integridad Referencial

Todas las relaciones utilizarán claves foráneas.

Las eliminaciones deberán respetar las siguientes reglas:
...
únicamente en tablas puente.

---

# 269. Índices Recomendados
...
search_vector (futuro)

---

# 270. Búsqueda

En futuras versiones se implementará búsqueda Full Text de PostgreSQL.
...
Todas las modificaciones importantes deberán registrar:
...
# 272. Row Level Security (RLS)

Supabase utilizará políticas RLS.

Las tablas públicas serán únicamente de lectura para visitantes.

Las operaciones de escritura estarán restringidas a usuarios autenticados con permisos administrativos.

Las tablas de contacto permitirán inserciones públicas controladas, pero no lectura por usuarios anónimos.

---

# 273. Escalabilidad

El modelo ha sido diseñado para soportar:

- Miles de productos.
- Miles de artículos.
- Decenas de miles de imágenes.
- Miles de solicitudes de contacto.
- Futuras funciones de e-commerce.
...
El diseño de la base de datos será considerado finalizado cuando:

- Todas las entidades principales estén definidas.
- Existan relaciones consistentes entre módulos.
- Se garantice la integridad referencial.
- Se contemplen índices para optimizar consultas frecuentes.
- Se incorporen políticas de seguridad mediante RLS.
- El modelo permita la incorporación de nuevas funcionalidades sin requerir rediseños estructurales significativos.
...
El presente diccionario de datos documenta la estructura física de las entidades principales del sistema.

Para cada tabla se especificarán:
...
Esta información servirá como referencia oficial durante el desarrollo del backend, la creación de migraciones y la implementación del panel administrativo.
...
Almacena todos los productos publicados en el catálogo de Poppy Crafty.
...
# 280. Tabla: product_tag_relations
...
Mantener consistencia entre claves.

Evitar nombres reservados.

Mantener restricciones de integridad.

---

# 288. Convenciones de Índices
...
Campos utilizados por buscadores.

Campos utilizados para ordenamiento.

---

# 289. Restricciones Globales

Todos los slugs deberán ser únicos.

Los precios nunca podrán ser negativos.

Las fechas de actualización deberán modificarse automáticamente.

Los registros archivados nunca serán eliminados físicamente.

---

# 290. Criterios de Aceptación

El diccionario de datos será considerado finalizado cuando:

- Todas las tablas principales estén completamente documentadas.
- Se definan los tipos de datos y restricciones para cada campo.
- Se especifiquen índices y claves primarias.
- Exista consistencia en la nomenclatura y estructura.
- La documentación permita implementar la base de datos sin ambigüedades.
...
# Parte 6.3
# Seguridad de la Base de Datos (Row Level Security - RLS)
...
Este capítulo define las políticas de seguridad que regirán el acceso a la base de datos del sistema Poppy Crafty.

Se utilizará el sistema **Row Level Security (RLS)** proporcionado por Supabase, permitiendo controlar qué operaciones puede realizar cada tipo de usuario sobre cada tabla.

El objetivo es garantizar la confidencialidad, integridad y disponibilidad de la información, minimizando riesgos de acceso no autorizado.

---

# 292. Principios de Seguridad

Todas las políticas deberán seguir los siguientes principios:

- Denegar por defecto.
- Conceder únicamente los permisos necesarios.
- Separar claramente el acceso público del acceso administrativo.
- Evitar exponer información sensible.
- Mantener trazabilidad de las operaciones.
- Preparar el sistema para múltiples roles en futuras versiones.

---

# 293. Tipos de Usuarios
...
Usuario autenticado con privilegios completos.
...
- Editar productos.
- Publicar productos.
- Eliminar lógicamente productos.
- Gestionar artículos.
...
Rol pensado para futuras versiones.

Podrá:

- Crear artículos.
- Editar artículos.
- Gestionar imágenes del blog.
- Publicar contenido.
- Administrar categorías del blog.

No podrá modificar configuraciones globales ni productos.

---

# 294. Política General

Todas las tablas tendrán activado Row Level Security.

Toda operación será denegada hasta que exista una política que la permita explícitamente.
...
Permitido únicamente para administradores.
...
No se permitirá eliminación física.

Solo actualización del estado a:

archived

---

# 296. Tabla: product_categories
...
# 297. Tabla: product_images

SELECT

Permitido para todos si el producto asociado está publicado.
...
Permitido para visitantes únicamente cuando:
...
Solo eliminación lógica.

---

# 299. Tabla: post_categories
...
Permitido públicamente mediante formulario.
...
Público únicamente para configuraciones visibles.
...
# 304. Políticas para Storage

Los buckets de Supabase Storage también deberán protegerse mediante políticas.
...
Reservado para imágenes enviadas mediante formularios.
...
Pública únicamente mediante formularios controlados.

---

# 305. Validaciones

Antes de permitir cualquier escritura se verificará:

- Usuario autenticado.
- Rol válido.
- Registro existente.
- Restricciones de integridad.
- Tamaño permitido de archivos.
- Tipo MIME permitido.
...
Todas las operaciones administrativas deberán registrar:
...
- Registro.
- Operación realizada.
- Dirección IP (si la arquitectura futura lo permite).

---

# 307. Prevención de Ataques

El sistema deberá contemplar medidas para prevenir:
...
# 308. Validación de Archivos

Los archivos enviados deberán cumplir con:
...
Los nombres originales nunca se utilizarán como identificadores en Storage.

---

# 309. Protección de Datos

La información personal obtenida mediante formularios deberá:

- Almacenarse de forma segura.
- No exponerse públicamente.
- Utilizar conexiones HTTPS.
- Cumplir con principios de privacidad.
- Ser accesible únicamente por usuarios autorizados.

---

# 310. Registro de Errores

Los errores de seguridad deberán registrarse sin revelar información sensible al usuario.

El sistema nunca mostrará:
...
La arquitectura de seguridad deberá permitir incorporar posteriormente:
...
- API Keys.
- Tokens temporales.
- Accesos mediante OAuth.
- Integraciones externas.

---

# 312. Criterios de Aceptación

El módulo de seguridad será considerado finalizado cuando:

- Todas las tablas tengan RLS habilitado.
- Existan políticas para lectura y escritura.
- Los visitantes únicamente puedan acceder a información pública.
- Los formularios funcionen sin comprometer la seguridad.
- Los archivos se almacenen de forma segura.
- Las políticas sean compatibles con futuras ampliaciones del sistema.
- Se garantice el principio de mínimo privilegio para todos los usuarios.
...
La Base de Datos constituye el núcleo del sistema Poppy Crafty.

Su propósito será almacenar de forma segura, consistente y escalable toda la información generada por el sitio web, incluyendo productos, categorías, clientes, solicitudes, artículos del blog, archivos multimedia, cotizaciones y futuras funcionalidades como comercio electrónico, inventario y pagos.

La arquitectura estará diseñada para soportar el crecimiento del emprendimiento durante varios años sin requerir rediseños estructurales.
...
- Prepararse para nuevos módulos.
- Mantener consistencia transaccional.
- Integrarse completamente con Supabase.

---

# 757. Motor de Base de Datos

El sistema utilizará PostgreSQL administrado mediante Supabase.
...
# 758. Principios de Diseño

La arquitectura seguirá los siguientes principios:

- Normalización hasta 3FN cuando sea apropiado.
- Desnormalización únicamente cuando represente mejoras importantes de rendimiento.
- Uso consistente de claves primarias UUID.
- Relaciones explícitas.
- Integridad mediante Foreign Keys.
- Soft Delete.
- Auditoría.
- Versionado donde sea necesario.

---

# 759. Convenciones de Nombres

Todas las tablas utilizarán nombres en inglés.
...
Todos los nombres estarán escritos:

- En minúsculas.
- Separados mediante guion bajo.
- Sin espacios.
- Sin caracteres especiales.

---

# 760. Convenciones para Columnas

Las columnas seguirán un estándar uniforme.
...
La mayoría de tablas incluirán:
...
Esto permitirá mantener consistencia entre módulos.

---

# 763. Soft Delete

Ningún registro será eliminado físicamente de forma inmediata.

Los registros marcarán:
...
La eliminación física quedará reservada para procesos administrativos específicos.

---

# 764. Auditoría

Las operaciones importantes generarán registros históricos.
...
# 765. Modelo Entidad-Relación

El sistema estará compuesto por múltiples dominios funcionales.
...
Cada dominio será documentado individualmente.

---

# 766. Organización por Dominios

Las tablas estarán agrupadas lógicamente.
...
Esta organización facilitará futuras migraciones hacia arquitecturas basadas en microservicios si fuese necesario.

---

# 767. Integridad Referencial

Todas las relaciones importantes utilizarán claves foráneas.

Las restricciones impedirán:

- Registros huérfanos.
- Relaciones inválidas.
- Referencias inexistentes.

Las acciones ON DELETE y ON UPDATE serán definidas según las necesidades de cada entidad.
...
También podrán utilizarse índices compuestos cuando representen mejoras significativas.

---

# 769. Restricciones

Cada tabla incorporará restricciones apropiadas.
...
Estas restricciones deberán aplicarse desde la base de datos y no únicamente desde la aplicación.
...
Esto garantizará consistencia ante fallos.

---

# 771. Funciones SQL

La arquitectura permitirá implementar funciones almacenadas para:
...
Las funciones estarán versionadas mediante migraciones.

---

# 772. Triggers

Se utilizarán triggers únicamente cuando aporten ventajas claras.
...
Sincronizar información derivada.

---

# 773. Vistas

Se crearán vistas para simplificar consultas frecuentes.
...
Las vistas nunca sustituirán la lógica de negocio.

---

# 774. Row Level Security (RLS)

Todas las tablas protegidas utilizarán políticas RLS.
...
La seguridad deberá implementarse desde la base de datos.
...
# 776. Preparación para Futuras Funcionalidades
...
Sin necesidad de rediseñar las entidades principales.

---

# 777. Estructura General del Modelo

El modelo de datos será documentado siguiendo el siguiente orden:
...
8.14 Tablas auxiliares

Cada dominio incluirá:
...
# 778. Requerimientos No Funcionales
...
# 779. Criterios de Aceptación

La arquitectura de la base de datos será considerada adecuada cuando:

- Mantenga la integridad de toda la información.
- Permita escalar el sistema durante varios años.
- Facilite el mantenimiento.
- Minimice redundancias.
- Optimice consultas frecuentes.
- Permita incorporar nuevos módulos sin afectar el modelo existente.

---

## Fin de la Introducción del Capítulo 8
...
Este capítulo define el diseño de la base de datos que soportará el funcionamiento del sitio web de Poppy Crafty.

El modelo de datos ha sido diseñado considerando los requerimientos funcionales y no funcionales definidos en este documento, priorizando la integridad de la información, la escalabilidad, el rendimiento y la facilidad de mantenimiento.

La base de datos deberá servir como soporte para los módulos del catálogo, blog, galería, solicitudes de clientes, panel administrativo y futuras funcionalidades como tienda en línea, gestión de pedidos y pagos.

---

## 17.2 Arquitectura General

El proyecto utilizará PostgreSQL administrado mediante Supabase como sistema gestor de base de datos.

La arquitectura se ha diseñado siguiendo un enfoque relacional, aprovechando las capacidades nativas de PostgreSQL para garantizar consistencia, seguridad y escalabilidad.

Durante el diseño se adoptan los siguientes principios:

- Uso de UUID como clave primaria en todas las tablas.
- Integridad referencial mediante claves foráneas.
- Separación entre autenticación y datos de perfil de usuario.
- Compatibilidad con Row Level Security (RLS).
- Compatibilidad con Supabase Storage.
- Uso de Soft Delete cuando sea necesario.
- Auditoría de operaciones críticas.
- Preparación para futuras ampliaciones sin modificar la estructura principal.

La autenticación será administrada por Supabase Auth, mientras que la información propia del negocio será almacenada en tablas independientes relacionadas mediante claves foráneas.

---

## 17.3 Modelo Entidad–Relación

El modelo general de la base de datos estará organizado en seis módulos principales.
...
Este modelo organiza la información por módulos funcionales, facilitando el mantenimiento, la reutilización del código y la futura incorporación de nuevas funcionalidades.

---

## 17.4 Convenciones del Modelo

Para mantener consistencia durante todo el desarrollo se adoptan las siguientes convenciones.

### Convención de nombres

- Todas las tablas utilizarán nombres en minúsculas.
- Se utilizará el formato `snake_case`.
- Las claves foráneas finalizarán con el sufijo `_id`.
- Los nombres deberán ser descriptivos y evitar abreviaturas.
...
Todas las tablas utilizarán un identificador UUID generado automáticamente.
...
Las relaciones entre tablas se implementarán mediante claves foráneas con integridad referencial.
...
Todas las tablas incluirán, como mínimo, los siguientes campos:
...
Las tablas cuyos registros puedan eliminarse de forma lógica incorporarán además:
...
Todos los campos de fecha utilizarán el tipo:

```sql
TIMESTAMPTZ
```

Esto permitirá almacenar fechas considerando zonas horarias y mantener consistencia entre servidor y clientes.

---

### Estados

Los estados simples se representarán mediante campos booleanos.
...
Cuando un módulo requiera múltiples estados, se utilizarán tipos ENUM de PostgreSQL.

---

### Slugs

Las entidades públicas del sitio deberán contar con un slug único para generar URLs amigables.
...
Los registros que puedan requerir recuperación no serán eliminados físicamente.

En su lugar se utilizará el campo:

```sql
deleted_at TIMESTAMPTZ NULL
```

La eliminación física quedará reservada únicamente para procesos administrativos específicos.

---

### Índices

Cada tabla contará únicamente con los índices necesarios para optimizar las consultas más frecuentes.

Se priorizarán índices sobre:
...
Se evitará crear índices innecesarios que incrementen el costo de escritura.

---

### Seguridad

La información pública y privada estará protegida mediante políticas Row Level Security (RLS).

Cada tabla definirá sus propias reglas de acceso según el rol del usuario.

---

### Escalabilidad

El modelo ha sido diseñado para permitir incorporar futuras funcionalidades sin modificar la estructura principal de la base de datos.
...
La incorporación de estos módulos podrá realizarse mediante nuevas tablas y relaciones, preservando la arquitectura existente.

---

## 17.5 Diccionario de Datos

Las siguientes secciones describen cada una de las tablas que conforman el modelo de datos del sistema, incluyendo su propósito, estructura, relaciones, restricciones e índices.
...
Almacena las categorías principales del catálogo de productos.
...
| slug | VARCHAR(120) | NOT NULL, UNIQUE | URL amigable. |
| descripcion | TEXT | NULL | Descripción opcional. |
| imagen_url | TEXT | NULL | Imagen representativa. |
| orden | INTEGER | DEFAULT 0 | Orden de visualización. |
| activo | BOOLEAN | DEFAULT TRUE | Estado de la categoría. |
...
- Las categorías inactivas no serán visibles en el sitio.
- No podrá eliminarse una categoría con productos asociados.
...
Representa el catálogo principal del emprendimiento.

Cada registro corresponde a un producto o servicio personalizable ofrecido por Poppy Crafty.
...
productos
 ...
 ┌────┴───────────────┐
 ▼                    ▼
producto_imagenes   producto_opciones
```
...
| categoria_id | UUID | FK | Categoría. |
| nombre | VARCHAR(150) | NOT NULL | Nombre del producto. |
| slug | VARCHAR(180) | UNIQUE | URL amigable. |
| descripcion_corta | TEXT | NULL | Resumen mostrado en tarjetas. |
| descripcion | TEXT | NULL | Información completa. |
| precio_desde | NUMERIC(10,2) | NULL | Precio inicial. |
| tiempo_entrega | VARCHAR(100) | NULL | Tiempo estimado de elaboración. |
| destacado | BOOLEAN | DEFAULT FALSE | Producto destacado. |
| publicado | BOOLEAN | DEFAULT TRUE | Visible para clientes. |
| seo_title | VARCHAR(70) | NULL | Meta título. |
| seo_description | VARCHAR(160) | NULL | Meta descripción. |
| created_at | TIMESTAMPTZ | NOT NULL | Fecha de creación. |
...
- Los productos eliminados utilizarán Soft Delete.
...
Almacena todas las fotografías asociadas a un producto.

Permite construir galerías de imágenes y seleccionar una imagen principal.
...
- Cada producto deberá tener al menos una imagen.
- Solo una imagen podrá marcarse como principal.
- El orden definirá la secuencia de visualización.

---

# Tabla: producto_opciones

## Descripción

Representa las opciones de personalización disponibles para un producto.

No almacena inventario.

Permite definir todas las configuraciones que el cliente puede seleccionar antes de realizar un pedido.
...
- Un producto podrá tener múltiples opciones.
- El incremento de precio será opcional.
- El orden controlará la visualización en el sitio.

# Módulo: Blog

El módulo de blog permitirá publicar contenido educativo y de valor relacionado con la personalización de productos, el uso de máquinas Cricut, consejos para emprendedores y novedades del emprendimiento.

Su principal objetivo es mejorar el posicionamiento SEO del sitio, atraer tráfico orgánico y fortalecer la presencia digital de Poppy Crafty.
...
Almacena las categorías utilizadas para clasificar los artículos del blog.
...
| updated_at | TIMESTAMPTZ | NOT NULL | Actualización. |

---

## Reglas de Negocio

- No podrán existir categorías repetidas.
- Solo las categorías activas aparecerán en el sitio.
...
Almacena todos los artículos publicados en el blog.

Cada artículo podrá pertenecer a una categoría y asociarse con múltiples etiquetas.
...
- Un artículo no podrá publicarse sin título.
- El slug deberá ser único.
- Solo los artículos publicados serán visibles para los visitantes.
- Los artículos eliminados utilizarán Soft Delete.
...
Almacena las etiquetas utilizadas para clasificar artículos.
...
- Las etiquetas no podrán duplicarse.
- Un mismo nombre generará un único registro.
...
Tabla puente que implementa la relación muchos a muchos entre artículos y etiquetas.
...
- Un artículo podrá tener múltiples etiquetas.
- Una etiqueta podrá utilizarse en múltiples artículos.
- No podrán existir registros duplicados para la misma combinación post-etiqueta.
